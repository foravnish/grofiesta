...
class OfferRepository {
    ...
    private var mRepository: OfferRepository? = null

    fun getInstance(): OfferRepository {
        if (mRepository == null) {
            mRepository = OfferRepository()
        }
        return mRepository as OfferRepository
    }
    ...
    fun callOfferList(
        ...
        val mLiveData = MutableLiveData<ApiResponseModels.OffersResponse>()
        if (NetworkHandling.isConnected(context)) {
            if (showDialog) (context as BaseActivity).showDialog()
            apiInterface!!.callOfferListing()
