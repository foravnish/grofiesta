...
class NetworkLogger : Interceptor {
    ...
    override fun intercept(chain: Interceptor.Chain): Response {
        ...
        /**Start---This is for common query in all api*/
        request.url.apply {
            newBuilder().apply {
//                addQueryParameter(
//                    "appName",
...
//                addQueryParameter("erpIDprm", AppPreference.getMyErpID(App.instance))
//                addQueryParameter("parentIDprm", AppPreference.getMyParentID(App.instance))
//                addQueryParameter("appversion", BuildConfig.VERSION_NAME)
            }.build().let {
                request.newBuilder().url(it).let {
                    request = it.build()
                    ...
                    when (response.code) {
                        400 -> {
                            ...
                            if (contentLength != 0L) {
                                ...
                                val jhfs = JSONObject(responseBodyString)
                                val url =request.url
//                                val ErrorAPiRepository= ErrorLogRepository().getInstance()
                                val sdh=responseHeaders.values("Authorization-Token")
                                var Token=""
                                for (item in sdh){
                                    Token=item
                                }
        ...
        /**End---This is for common query in all api*/
